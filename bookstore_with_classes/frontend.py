...
def search_command():
    list1.delete(0,END)
    for row in database.search(title_text.get(),author_text.get(),isbn_value.get(),isbn_value.get()):
        list1.insert(END,row)

# Add entry function
def add_command():
    database.insert(title_text.get(),author_text.get(),year_value.get(),isbn_value.get())
    list1.delete(0,END)
...
# Add delete function
def update_command():
    database.update(selected_tuple[0],title_text.get(),author_text.get(),year_value.get(),isbn_value.get())
